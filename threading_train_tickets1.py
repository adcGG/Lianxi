...
if __name__=='__main__':
	...
	for j in range(0, 100):
		t.append(threading.Thread(target=buy_ticket2, args=(name[j], nums[j], date1[j], start_station[j])))
	for j in range(0, 100):
		t[j].start()
	# for k in range(0,30001):
	for j in range(0, 100):
		t[j].join()



	# t1 = threading.Thread(target=buy_ticket,args=(name[j],nums[j],date1[j],start_station[j]))
